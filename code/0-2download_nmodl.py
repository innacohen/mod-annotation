...
for _, row in tqdm(filtered_df.iterrows(), total=len(filtered_df), desc="Downloading MOD files"):
    try:
        ...
        success, content = download_file(url, output_path)
                ...
                if include_files:
                    ...
                    for include_file in include_files:
                        ...
                        if include_url:
                            ...
                            include_success, _ = download_file(include_url, include_path)
                            ...
                            if include_success:
                                ...
                                logging.error(f"Failed to download include file: {include_file} from URL: {include_url}")
                        else:
                            failed_include_count += 1
                            logging.error(f"Could not generate URL for include file: {include_file} from MOD URL: {url}")
        else:
...
if failed_mod_count > 0 or failed_include_count > 0:
